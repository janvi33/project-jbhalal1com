...
# def handle_client_connection(client_socket, private_key, public_key_path, credentials_file, balance_file):
#     try:
#         authenticated = False
#         user_id = None
...
#             symmetric_key = decrypt_with_private_key(private_key, client_socket.recv(1024))
#             request = decrypt_with_private_key(symmetric_key, client_socket.recv(1024)).decode()
...
#             with open(public_key_path, "rb") as public_key_file:
#                 public_key_bytes = public_key_file.read()
#                 client_sock.sendall(public_key_bytes)

#             client_handler = threading.Thread(
#                 target=handle_client_connection,
#                 args=(client_sock, private_key, public_key_path, credentials_file, balance_file)
#             )
...
def decrypt_with_private_key_and_aes(private_key, encrypted_symmetric_key, encrypted_message):
    ...
    # The following line returns a string, so no need to call decode() on it
